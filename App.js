...
export default function App() {
  ...
  const finisheding = (key) => setFinished(!finished);
  ...
  const finishedToDo = (key) => {
          const newToDos = { 
            ...toDos,
            [key] : { ...toDos[key], ['finished']: !finished }
          };
          ...
          (key) =>
            toDos[key].working === working ? (
              <View style={styles.toDo} key={key}>
                ...
                {/* 수정모드일시, ㄴ이곳이 Text가 아닌 text input으로 보여지게 할 것 */}
                <View style={styles.btnBox}>
                  
                  <TouchableOpacity onPress={() => deleteToDo(key)}>
                    <Fontisto name="trash" size={18} color={theme.btn} />
                  </TouchableOpacity>

                  <TouchableOpacity style={{flexDirection: "row", marginLeft:10,}} onPress={() => finishedToDo(key)}>
                  {(toDos[key].finished ?
                    <Fontisto name="checkbox-active" style={{paddingTop:4}} size={18} color={theme.btn} />
                    :  <MaterialCommunityIcons name="checkbox-blank-outline" size={25} color={theme.btn} /> )}
                  </TouchableOpacity>
